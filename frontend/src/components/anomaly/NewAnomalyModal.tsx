...
import { ArrowLeft, ArrowRight, Send, Upload, CheckCircle, Loader2, X, AlertTriangle, Minus, Plus } from "lucide-react";
import { cn } from "@/lib/utils";
import { useAnomalyMutations, useSuggestPriorityMutation } from "@/hooks/useAnomalies";
...
export default function NewAnomalyModal({ isOpen, onClose, onSuccess }: NewAnomalyModalProps) {
  ...
  const handleNext = () => {
    ...
    if (currentStep === 2) {
      ...
      // Call the ML API to get priority suggestion
      suggestPriority.mutate(
        {
          description_anomaly: formData.anomaly_description,
          description_equipement: formData.equipment_description,
        },
        {
          onSuccess: (data) => {
            setFormData((prev) => ({
              ...prev,
              process_safety: data.process_safety,
              fiabilite_integrite: data.fiabilite_integrite,
          ...
          onError: (error) => {
            ...
            setIsLoading(false);
            // Optionally handle error UI or rollback step
          },
        }
      );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const anomalyData: AnomalyFormData = {
        ...
        comment: [formData.equipment_description ? `Equipment: ${formData.equipment_description}` : "", formData.comment || ""].filter(Boolean).join("\n\n"),
      };
    ...
    <div className="flex items-center justify-center mb-6">
      {[1, 2, 3, 4].map((step) => (
        <div key={step} className="flex items-center">
          <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all duration-300", step <= currentStep ? "bg-blue-600 text-white shadow-lg" : "bg-gray-200 text-gray-500")}>{step < currentStep ? <CheckCircle className="w-4 h-4" /> : step}</div>
          {step < 4 && <div className={cn("w-12 h-0.5 mx-2 transition-all duration-300", step < currentStep ? "bg-blue-600" : "bg-gray-200")} />}
        </div>
      ))}
    </div>
  );
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-300">
        <div className="sticky top-0 bg-white border-b border-gray-200 rounded-t-2xl p-6">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={handleClose} className="h-8 w-8 p-0 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full">
              <X className="h-4 w-4" />
            </Button>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            {createAnomaly.isError && (
              <Alert className="border-red-200 bg-red-50">
                ...
                <AlertDescription className="text-red-800">{(createAnomaly.error as any)?.response?.data?.message || (createAnomaly.error as Error)?.message || "An error occurred while creating the anomaly."}</AlertDescription>
              </Alert>
            )}
            ...
            {currentStep === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea id="anomaly_description" placeholder="Describe the anomaly in detail..." value={formData.anomaly_description} onChange={(e) => handleInputChange("anomaly_description", e.target.value)} rows={4} className={cn("mt-1", errors.anomaly_description && "border-red-500")} />
                  {errors.anomaly_description && <p className="text-red-500 text-xs mt-1">{errors.anomaly_description}</p>}
                </div>
                ...
                <div>
                  ...
                  <Textarea id="equipment_description" placeholder="Describe the equipment in detail..." value={formData.equipment_description} onChange={(e) => handleInputChange("equipment_description", e.target.value)} rows={4} className={cn("mt-1", errors.equipment_description && "border-red-500")} />
                  {errors.equipment_description && <p className="text-red-500 text-xs mt-1">{errors.equipment_description}</p>}
                </div>
                ...
                <div>
                  ...
                  <Input id="date_detection" type="date" defaultValue={new Date().toISOString().split("T")[0]} onChange={(e) => handleInputChange("date_detection", e.target.value)} className={cn("mt-1", errors.date_detection && "border-red-500")} />
                  {errors.date_detection && <p className="text-red-500 text-xs mt-1">{errors.date_detection}</p>}
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input id="equipment_number" type="text" placeholder="Enter equipment number" value={formData.equipment_number} onChange={(e) => handleInputChange("equipment_number", e.target.value)} className="mt-1" />
                </div>
                ...
                <div>
                  ...
                  <Input id="system_number" type="text" placeholder="Enter system number" value={formData.system_number} onChange={(e) => handleInputChange("system_number", e.target.value)} className="mt-1" />
                </div>
                ...
                <div>
                  ...
                  <Input id="comment" type="text" placeholder="Enter any additional comments" value={formData.comment} onChange={(e) => handleInputChange("comment", e.target.value)} className="mt-1" />
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-3">
                    <Input id="modal-file" type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleModalFileChange} className="hidden" />
                    <Button type="button" variant="outline" onClick={() => document.getElementById("modal-file")?.click()} className="flex items-center gap-2 hover:bg-gray-50">
                      <Upload className="w-4 h-4" />
                      Choose File
                    </Button>
                    {formData.file && <span className="text-sm text-gray-600 bg-gray-50 px-3 py-1 rounded-full">{formData.file.name}</span>}
                  </div>
            ...
            {currentStep === 3 && (
              <div className="text-center py-16">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600">Analyzing your anomaly data to generate criticality scores...</p>
                </div>
              </div>
            )}
            ...
            {currentStep === 4 && (
              <div className="space-y-6">
                <div className="text-center mb-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">AI-Generated Criticality Assessment</h3>
                  <p className="text-sm text-gray-600">Our AI system has analyzed your anomaly and generated the following criticality scores:</p>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm font-semibold text-gray-900 leading-relaxed bg-white px-3 py-2 rounded-md border border-gray-100 line-clamp-1">{formData.anomaly_description || "-"}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 p-2 rounded-lg border border-gray-200/60 bg-white">
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("fiabilite_integrite", Math.max(1, formData.fiabilite_integrite - 1))} disabled={formData.fiabilite_integrite <= 1} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("fiabilite_integrite", Math.min(5, formData.fiabilite_integrite + 1))} disabled={formData.fiabilite_integrite >= 5} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 p-2 rounded-lg border border-gray-200/60 bg-white">
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("disponibilite", Math.max(1, formData.disponibilite - 1))} disabled={formData.disponibilite <= 1} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                        <Minus className="h-3 w-3" />
                      </Button>
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("disponibilite", Math.min(5, formData.disponibilite + 1))} disabled={formData.disponibilite >= 5} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                        <Plus className="h-3 w-3" />
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 p-2 rounded-lg border border-gray-200/60 bg-white">
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("process_safety", Math.max(1, formData.process_safety - 1))} disabled={formData.process_safety <= 1} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                        <Minus className="h-3 w-3" />
                      </Button>
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleCriticalityChange("process_safety", Math.min(5, formData.process_safety + 1))} disabled={formData.process_safety >= 5} className="h-6 w-6 p-0 text-gray-400 hover:text-gray-600 hover:bg-white/80">
                        <Plus className="h-3 w-3" />
                      </Button>
                    </div>
                  </div>
            ...
            <div className="flex justify-between pt-6 border-t">
              {currentStep !== 4 && (
                <Button variant="outline" onClick={handlePrevious} disabled={currentStep === 1 || currentStep === 3} className="flex items-center gap-2">
              ...
              <div className={cn("flex gap-2", currentStep === 4 && "ml-auto")}>
                ...
                {currentStep === 4 && (
                  <Button onClick={handleSubmit} disabled={createAnomaly.isPending} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700">
